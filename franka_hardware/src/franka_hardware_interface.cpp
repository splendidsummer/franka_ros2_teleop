...
std::vector<CommandInterface> FrankaHardwareInterface::export_command_interfaces() {
  ...
  // Register cartesian pose command interfaces
  RCLCPP_INFO(getLogger(), "Register cartesian pose command interfaces");
  for (auto i = 0U; i < hw_cartesian_pose_commands_.size(); i++) {
    command_interfaces.emplace_back(
        CommandInterface(std::to_string(i), k_HW_IF_CARTESIAN_POSE_COMMAND,
                         &hw_cartesian_pose_commands_.at(i)));

    RCLCPP_INFO(getLogger(),
                "Registering cartesian pose command interface: %s/%s with index %d",
                std::to_string(i).c_str(), k_HW_IF_CARTESIAN_POSE_COMMAND.c_str(), i);
  }

  // Register elbow command interfaces
  RCLCPP_INFO(getLogger(), "Register elbow command interfaces");
  for (auto i = 0U; i < hw_elbow_command_.size(); i++) {
    command_interfaces.emplace_back(
        CommandInterface(hw_elbow_command_names_.at(i), k_HW_IF_ELBOW_COMMAND,
                         &hw_elbow_command_.at(i)));

    RCLCPP_INFO(getLogger(),
                "Registering elbow command interface: %s/%s with index %d",
                hw_elbow_command_names_.at(i).c_str(), k_HW_IF_ELBOW_COMMAND.c_str(), i);
  }
