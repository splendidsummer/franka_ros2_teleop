...
GripperActionServer::GripperActionServer(const rclcpp::NodeOptions& options)
  ...
  try {
    // Note: The gripper is initialized here
  ...
  // Note:: The gripper state is reading from libfranka here! 
  current_gripper_state_ = gripper_->readOnce();
  // Note::   enum class Task { kHoming, kMove, kGrasp, kGripperCommand };
  ...
  // Note:: using Homing = franka_msgs::action::Homing;  
  this->homing_server_ = rclcpp_action::create_server<Homing>(
      this, "~/homing",
      /**
       * 捕获列表 `[this, kHomingTask]`*  
          - `this`：捕获当前对象的指针，以便在 Lambda 内部调用成员函数（如 `handleGoal`）或访问成员变量。  
          - `kHomingTask`：捕获局部变量 `kHomingTask`（这里是一个 `Task::kHoming` 枚举值）作为只读副本，用来告诉回调当前要处理哪种任务。
       * 函数体*  - 直接调用成员函数 `handleGoal(kHomingTask)`，将捕获的任务类型传进去，并返回一个 `GoalResponse`。
       */
...
void GripperActionServer::executeMove(const std::shared_ptr<GoalHandleMove>& goal_handle) {
  auto command = [goal_handle, this]() {
    ...
    // Note:: the gripper is built from libfranka, therefore the "move" action control is actually from the "libfranka" lib
